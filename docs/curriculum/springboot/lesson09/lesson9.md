...
- H2でのLesson9実行は `flyway-core` が担当します。
- 後続のMariaDB演習では `flyway-mysql` が必要です。Lesson9の成果物をそのままLesson6/7へ渡すため、この時点で追加します。
...
`lesson09/data` は実行時DBなのでGit管理しません。`lesson09/.gitignore` に次を追加します。
...
- `~/order-management-springboot/stages/lesson09/src/main/resources/db/migration/V2__add_index_to_attendance_work_date.sql`
...
作成ファイル:
- `~/order-management-springboot/stages/lesson09/src/test/java/com/shinesoft/attendance/MigrationSmokeTest.java`
