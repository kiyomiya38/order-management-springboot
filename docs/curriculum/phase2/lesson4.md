# Phase2 Lesson4: 複数ユーザー切り替えロジック（Webログイン前提）

## ロードマップ接続
- 対象: `springless-roadmap.md` の Phase2
- このLessonで増える機能: 業務ルール違反ケースの強化（未出勤退勤/二重出勤）
- Phase完了時に到達する機能: 打刻業務ルールを画面操作に接続して説明できる
- 先に読む `src`: service/AttendanceService, domain/Attendance, exception/BusinessException
...
- `App.java` に今回の差分を実装する: 業務ルール違反ケースの強化（未出勤退勤/二重出勤）
