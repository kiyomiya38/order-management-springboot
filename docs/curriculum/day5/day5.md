...
理解ポイント（15分）:
- この手順の意図:
  - Day5は差分ファイルが多く、手入力ミスを減らすため完成版を転記する
- 学習上の注意:
  - 転記して終わりにせず、必ず `SecurityConfig` / `UserService` / `AdminAttendanceController` を読んで役割を把握する
- 重点読解ファイル:
  - `config/SecurityConfig.java`（認証と権限）
  - `service/UserService.java`（ユーザー管理）
  - `web/AdminAttendanceController.java`（管理者勤怠編集）
...
- このチェックの目的:
  - 必須ファイルの転記漏れを起動前に潰す
- 重要ポイント:
  - Day5は「依存追加 + 多数ファイル追加」のため、起動前チェックが最短ルート
