...
2. この資料では Maven や Gradle で追加する外部ライブラリは使わない
3. `import` は `java.lang` 以外のパッケージにあるクラスを短く書くための宣言
4. 便利メソッドを使うと、自作コードを減らせる
5. `isBlank()` は入力チェックで「未入力または空白だけ」の値を判定するときに使う
6. `trim()` はデータ整形で文字列の前後空白を除去し、保存や比較を安定させるときに使う
