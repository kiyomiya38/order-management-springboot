# Java-18 ハンズオン: コレクション（List / Set / Map）

対応参考資料: `Java-18_コレクション.pptx`

## 1. この資料のゴール
- `List`, `Set`, `Map` の違いを説明できる
- 代表実装（`ArrayList`, `HashSet`, `HashMap`）を使える
- 実務データの保持に適したコレクションを選べる

---

## 2. 事前準備
```bash
cd ~/order-management-springboot/practice/java
java -version
javac -version
```

---

## 3. 先に覚えるポイント
1. `List`: 順序あり・重複あり
2. `Set`: 重複なし
3. `Map`: キーと値の対応

---

## 4. ハンズオン

目的:
- 実務で頻出のコレクションを使い分ける

完了条件:
- `CollectionDemo.java` で `List` / `Set` / `Map` を実行できる

作成ファイル: `~/order-management-springboot/practice/java/handson18/CollectionDemo.java`

### Step 0: 作業フォルダを作る
```bash
mkdir -p ~/order-management-springboot/practice/java/handson18
cd ~/order-management-springboot/practice/java/handson18
```

### Step 1: List を使う
`CollectionDemo.java` を次の内容で作成:

```java
import java.util.ArrayList;
import java.util.List;

public class CollectionDemo {
    public static void main(String[] args) {
        List<String> productCodes = new ArrayList<>();
        productCodes.add("P-001");
        productCodes.add("P-002");
        productCodes.add("P-002"); // 重複あり

        System.out.println("件数: " + productCodes.size());
        for (String code : productCodes) {
            System.out.println(code);
        }
    }
}
```

実行:
```bash
javac -encoding UTF-8 CollectionDemo.java
java CollectionDemo
```

期待結果:
- `javac` がエラーなく完了する
- `java` の実行結果が、このStepのコード内容と一致する


### Step 2: Set を使う
`CollectionDemo.java` を次の内容に更新:

```java
import java.util.HashSet;
import java.util.Set;

public class CollectionDemo {
    public static void main(String[] args) {
        Set<String> tags = new HashSet<>();
        tags.add("PAID");
        tags.add("PAID");
        tags.add("URGENT");

        System.out.println("件数(重複除去): " + tags.size());
        System.out.println(tags);
    }
}
```

実行:
```bash
javac -encoding UTF-8 CollectionDemo.java
java CollectionDemo
```

期待結果:
- `javac` がエラーなく完了する
- `java` の実行結果が、このStepのコード内容と一致する


### Step 3: Map を使う（仕上げ）
`CollectionDemo.java` を次の内容に更新:

```java
import java.util.HashMap;
import java.util.Map;

public class CollectionDemo {
    public static void main(String[] args) {
        Map<String, Integer> stockByCode = new HashMap<>();
        stockByCode.put("P-001", 12);
        stockByCode.put("P-002", 5);

        System.out.println("P-001 在庫: " + stockByCode.get("P-001"));
        System.out.println("P-003 在庫あり? " + stockByCode.containsKey("P-003"));

        for (Map.Entry<String, Integer> entry : stockByCode.entrySet()) {
            System.out.println(entry.getKey() + " -> " + entry.getValue());
        }
    }
}
```

実行:
```bash
javac -encoding UTF-8 CollectionDemo.java
java CollectionDemo
```

期待結果:
- `javac` がエラーなく完了する
- `java` の実行結果が、このStepのコード内容と一致する


---

## 5. ミニ演習（10分）
1. `List` に商品名を5件追加して表示
2. `Set` で重複データが消えることを確認
3. `Map` に `put` で同じキーを入れたときの上書きを確認

---

## 6. つまずきポイント
- `import` 漏れでコンパイルエラー
  -> `java.util` の `import` を確認
- `Map.get` が `null`
  -> キー存在確認に `containsKey` を使う
- コレクション選定ミス
  -> 順序・重複・キー検索の要件で選ぶ
