...
class LoggingAttendanceRepository implements AttendanceRepository {
    private final AttendanceRepository delegate;

    LoggingAttendanceRepository(AttendanceRepository delegate) {
        this.delegate = delegate;
    }

    public Attendance findOrCreate(Long userId, LocalDate date) {
        System.out.println("[ATTENDANCE-REPO] findOrCreate userId=" + userId + ", date=" + date);
        return delegate.findOrCreate(userId, date);
    }

    public List<Attendance> findAllByDate(LocalDate date) {
        System.out.println("[ATTENDANCE-REPO] findAllByDate date=" + date);
        return delegate.findAllByDate(date);
    }
}
...
class HomeController {
    ...
    void getHome(HttpExchange exchange) throws IOException {
        ...
        String html = """
                <!doctype html>
                <html lang="ja">
                <head><meta charset="UTF-8"><title>Phase4 Lesson4</title></head>
                <body>
                  <h1>勤怠打刻</h1>
                  <p>ログイン中: %s</p>
...
class AppConfig {
    ...
    AppConfig() {
        ...
        AttendanceRepository baseRepo = new InMemoryAttendanceRepository();
        boolean enableRepoLog = true;
        this.attendanceRepository = enableRepoLog ? new LoggingAttendanceRepository(baseRepo) : baseRepo;
